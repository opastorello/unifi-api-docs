# UniFi Protect API - v5.3.38 - Referência

> Espelho automático de [`developer.ui.com/protect/v5.3.38`](https://developer.ui.com/protect/v5.3.38).
> OpenAPI `3.1.0` · 35 operações em 25 paths · atualizado na origem em `2026-06-02T09:19:13.019Z`.
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/meta/info" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/meta/info" \
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/viewers/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/viewers/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X PATCH "https://$UNIFI_HOST/proxy/protect/v1/viewers/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X PATCH "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/viewers/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/viewers" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/viewers" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
- `slots` **(obrigatório)**: `array` - List of cameras visible in each given slot. And cycling settings for each slot if it has multiple cameras listed.
  - _array de_ `object`:
    ...
    - `cycleInterval` **(obrigatório)**: `number` - How long should each camera stream be shown for in milliseconds until we cycle to the next camera
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/liveviews/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/liveviews/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X PATCH "https://$UNIFI_HOST/proxy/protect/v1/liveviews/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X PATCH "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/liveviews/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/liveviews" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/liveviews" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/liveviews" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/liveviews" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
      ...
      - _um de (modelKey):_
        ...
        - **variante**:
          ...
          - `ledSettings` **(obrigatório)**: `object` - LED settings.
            - `isEnabled` **(obrigatório)**: `boolean` - Whether the LED is enabled.
          ...
          - `activePatrolSlot` **(obrigatório)**
            - _um de (variantes):_
              ...
              - **variante**:
                - `null`
          - `videoMode` **(obrigatório)**: `string` enum: default, highFps, sport, slowShutter - Current video mode of the camera
        ...
        - **variante**:
          - `id` **(obrigatório)**: `string` - The primary key of device
          - `modelKey` **(obrigatório)**: `string` - The model key of the doorlock
          - `state` **(obrigatório)**: `string` enum: CONNECTED, CONNECTING, DISCONNECTED - Connection state of the device.
          - `name` **(obrigatório)**
            - _um de (variantes):_
              - **variante**:
    ...
    - `item` **(obrigatório)**
      - _um de (modelKey):_
        ...
        - **variante**:
          ...
          - `activePatrolSlot`
            - _um de (variantes):_
              ...
              - **variante**:
                - `null`
          - `videoMode`: `string` enum: default, highFps, sport, slowShutter - Current video mode of the camera
        ...
        - **variante**:
          ...
          - `batteryStatus`: `object` - Battery status.
            - `percentage`: `number|null` - Battery charge level from 0 to 100 (%).
            - `isLow`: `boolean` - Low battery charge level flag.
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/subscribe/devices" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/subscribe/devices" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
        ...
        - **variante**:
          ...
          - `metadata` **(obrigatório)**: `object`
            - `sensorType` **(obrigatório)**: `object` - Name of the metric measured by the sensor
              - `text` **(obrigatório)**: `string` enum: temperature, light, humidity
        ...
        - **variante**:
          ...
          - `metadata` **(obrigatório)**: `object`
            - `alarmType` **(obrigatório)**: `object` - A type of sensor alarm
              - `text` **(obrigatório)**: `string` enum: smoke, CO
        - **variante**:
          - `id` **(obrigatório)**: `string` - The primary key of event
          - `modelKey` **(obrigatório)**: `string` - The model key of the event
          - `type` **(obrigatório)**: `string`
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/subscribe/events" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/subscribe/events" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/ptz/patrol/start/{slot}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/ptz/patrol/start/{slot}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/ptz/patrol/stop" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/ptz/patrol/stop" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/ptz/goto/{slot}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/ptz/goto/{slot}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/alarm-manager/webhook/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/alarm-manager/webhook/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/lights/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/lights/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X PATCH "https://$UNIFI_HOST/proxy/protect/v1/lights/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X PATCH "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/lights/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/lights" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/lights" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
- `featureFlags` **(obrigatório)**: `object`
  ...
  - `videoModes` **(obrigatório)**: `array` - A list of supported video modes by the camera
    - _array de_ `string`:
      - `string` enum: default, highFps, homekit, sport, slowShutter
  - `hasMic` **(obrigatório)**: `boolean` - Whether the camera has a microphone
  - `hasLedStatus` **(obrigatório)**: `boolean` - Whether the camera has LED status
  - `hasSpeaker` **(obrigatório)**: `boolean` - Whether the camera has a speaker to support talkback
- `smartDetectSettings` **(obrigatório)**: `object` - Smart detection settings for the camera.
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
- `ledSettings`: `object` - LED settings.
  - `isEnabled`: `boolean` - Whether the LED is enabled.
...
```bash
# Local
curl -X PATCH "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X PATCH "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/cameras" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/rtsps-stream" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/rtsps-stream" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/rtsps-stream" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/rtsps-stream" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
...
```bash
# Local
curl -X DELETE "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/rtsps-stream" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X DELETE "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/rtsps-stream" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/snapshot" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/snapshot" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/disable-mic-permanently" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/disable-mic-permanently" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/cameras/{id}/talkback-session" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/cameras/{id}/talkback-session" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/sensors/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/sensors/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X PATCH "https://$UNIFI_HOST/proxy/protect/v1/sensors/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X PATCH "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/sensors/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/sensors" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/sensors" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/nvrs" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/nvrs" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/files/{fileType}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/files/{fileType}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X POST "https://$UNIFI_HOST/proxy/protect/v1/files/{fileType}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X POST "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/files/{fileType}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/chimes/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/chimes/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
...
```bash
# Local
curl -X PATCH "https://$UNIFI_HOST/proxy/protect/v1/chimes/{id}" \
     -H "X-API-KEY: $UNIFI_API_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
```bash
# Remoto (Cloud Connector)
curl -X PATCH "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/chimes/{id}" \
     -H "X-API-Key: $UNIFI_SM_KEY" \
     -H "Content-Type: application/json" -d '{ ... }'
```
</details>
...
```bash
# Local
curl -X GET "https://$UNIFI_HOST/proxy/protect/v1/chimes" \
     -H "X-API-KEY: $UNIFI_API_KEY"
```
```bash
# Remoto (Cloud Connector)
curl -X GET "https://api.ui.com/v1/connector/consoles/$CONSOLE_ID/protect/v1/chimes" \
     -H "X-API-Key: $UNIFI_SM_KEY"
```
</details>
